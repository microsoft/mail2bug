...
namespace Mail2Bug.Helpers
{
    public class CredentialsHelper  
    {
        ...
        public string GetPassword(string dpapiFilePath, Config.KeyVaultSecret secretsTuple)
        {
            var task = GetPasswordAsync(dpapiFilePath, secretsTuple);
            task.Wait();
            if (task.IsFaulted)
            {
        ...
        public async Task<string> GetPasswordAsync(string dpapiFilePath, Config.KeyVaultSecret secretsTuple)
        {
            if (secretsTuple == null)
            {
                ...
                return DPAPIHelper.ReadDataFromFile(dpapiFilePath);
            }
